...
public interface BookTransactionMapper {
    @Select("select tbt.id ,tbt.code ,tbt.from_date ,tbt.to_date,tbt.rent_status as rentType ,tm.name memberName,tb.name bookName from tbl_book_transaction tbt inner join tbl_member "+
            "tm on tbt.memberid =tm.memberid inner join tbl_book tb on tb.id =tbt.book_id where deleted =false")
...
@Select("select tbt.id ,tbt.code ,tbt.from_date ,tbt.to_date,tbt.rent_status as rentType ,tm.name as memberName ,tb.name as bookName from tbl_book_transaction tbt inner join tbl_member "+
        "tm on tbt.memberid =tm.memberid inner join tbl_book tb on tb.id =tbt.book_id  where tbt.id =#{id} and deleted =false")
